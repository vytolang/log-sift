...
SRC = src/main.vt src/record.vt src/tstamp.vt src/dedupe.vt src/filter.vt \
      src/source.vt src/native/src/io_shim.c
...
check-vytoc:
	@if [ -z "$(VYTOC)" ] || [ ! -x "$(VYTOC)" ]; then \
		echo "log-sift: cannot find the Vyto compiler."; \
		echo ""; \
...
DISTNAME = log-sift-$(VERSION)-$(PLATFORM)
...
dist: release
	...
	cd dist && cp $(DISTNAME).tar.gz log-sift-$(PLATFORM).tar.gz
	cd dist && sha256sum log-sift-$(PLATFORM).tar.gz > log-sift-$(PLATFORM).tar.gz.sha256
